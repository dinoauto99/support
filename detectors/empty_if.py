...
class EmptyIfDetector(BaseDetector):
                        ...
                        if stmt_brace_end != -1:
                            ...
                            if len(inner_block.strip()) == 0:
                                incidents.append(Incident(
                                    ...
                                    incident_type="Empty If",
                                    description="Found empty if block."
                                ))
                    elif body[next_char_idx] == ';':
                        incidents.append(Incident(
                            file_name=context.file_name,
                            function_name=context.function_name,
                            line_num=stmt_line_num,
                            line_code=stmt_line_code,
                            incident_type="Empty If",
                            description="Found empty if statement ending with semicolon."
                        ))
        return incidents
